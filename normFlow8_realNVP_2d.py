# Program implementing Real-NVP for toy 2d data
# note that this implementation does not involve the multiscale architecture with squeeze and split operation since we dont work with images in this demo
# check realnvp_images for the version with mutiscale architecture
...
class AffineCoupling(nn.Module):
    ...
    def forward(self, z):
        ...
        log_det_jacobian = torch.sum(torch.sum(torch.log(torch.abs(multiplier)), dim=1))
...
foldername = 'normFlow8_RealNVP_2d_flowLength=' + str(flow_length) + '_lr=' + str(lr) + '_bz=' + str(batch_size)
