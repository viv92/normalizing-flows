# Program demonstrating change of variable from a 2D uniform distribution to a 2D gaussian distribution using a planar flow as the transform
...
u = torch.tensor([1., 1.], requires_grad=True)
...
# transform function (planar flow) - must approximate the inverse of the cdf of the gaussian (after training is converged)
def transform_fn(z):
    a = torch.dot(w,z) + b
    a = sigmoid(a)
    x = z + u*a
