...
def lossfn(x, qz, T, params_mu, params_scale_tril):
    ...
    for i in range(N):
        ...
        z = z.squeeze(0)
        loss += -qz.log_prob(z) + torch.log(torch.abs(torch.det(params_cov))) # --- analytic jacobian = covariance_matrix
    return loss
...
print('time taken (analytic jacobian): ', et-st)
