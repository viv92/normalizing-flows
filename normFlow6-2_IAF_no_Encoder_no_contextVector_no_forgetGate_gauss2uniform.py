...
px = tdist.Uniform(torch.Tensor([0, 0]), torch.Tensor([1, 1]))
...
sns.kdeplot(x_samples[:,0], x_samples[:,1], label='target_distribution')
sns.kdeplot(xhat_samples[:,0], xhat_samples[:,1], label='transformed_distribution')

ax1.legend()
ax2.legend()
plt.savefig('normFlow6-2_gauss2uniform_flowLength='+str(flow_length)+'.png')
